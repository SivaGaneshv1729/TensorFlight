...
function StatCard({ icon: Icon, label, value, unit, color = 'text-white', alert = false }) {
  return (
    <div className={`bg-black/30 border rounded-md p-2.5 flex flex-col justify-center relative overflow-hidden ${alert ? 'border-red-500/60 bg-red-900/10' : 'border-gray-700/50'}`}>
      <div className="flex items-center gap-1.5 text-gray-400 text-[8px] mb-1 font-bold uppercase tracking-widest">
        <Icon size={10} /> {label}
      </div>
      <div className={`font-black text-lg leading-none ${color}`}>{value}</div>
      {unit && <div className="text-[8px] text-gray-600 mt-0.5">{unit}</div>}
    </div>
  )
}
...
export default function TelemetryPanel() {
  ...
  return (
    <div className="h-full flex flex-col font-sans min-h-0 gap-2">
      {/* Header */}
      ...
      <div className="flex-1 flex gap-3 min-h-0">
        ...
        {/* Right: AI Stats Grid */}
        <div className="flex-[1.35] grid grid-cols-2 gap-2 min-h-0">
          ...
          <div className={`col-span-2 rounded-md border p-2 flex items-center gap-2 ${ai.spray_recommended ? 'border-amber-500/50 bg-amber-900/10' : 'border-gray-700/30 bg-black/20'}`}>
            {ai.spray_recommended
              ? <><ShieldAlert size={14} className="text-amber-400 shrink-0" /><div><div className="text-[9px] font-black text-amber-400 uppercase tracking-widest">Spray Recommended</div><div className="text-[8px] text-gray-500">{ai.spray_zone_count} zones targeted · NDVI {(ai.vari_mean ?? 0).toFixed(3)}</div></div></>
              : <><ShieldCheck size={14} className="text-agri-secondary shrink-0" /><div><div className="text-[9px] font-black text-agri-secondary uppercase tracking-widest">No Spray Needed</div><div className="text-[8px] text-gray-500">VARI {(ai.vari_mean ?? 0).toFixed(3)} · ExG {(ai.exg_mean ?? 0).toFixed(3)} · NGRDI {(ai.ngrdi_mean ?? 0).toFixed(3)}</div></div></>
            }
          </div>
        </div>
      </div>
